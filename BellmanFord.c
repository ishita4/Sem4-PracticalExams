...
int negcycle(int E) //After all edges are relaxed |V-1|times, if any edge can be further relaxed, negative weight cycle exists
{
  for(int i = 0;i<E;i++)
  {
...
int main(void) {
 int V,E,s;  //V = no.of Vertices, E = no.of Edges, S is source vertex
  ...
  for (int i = 0;i<=V-1;i++)
  {
    if (i ==s)
      continue;
    printf("path from %d to %d is ",s,i+1);
    printpath(s-1,i);
    printf("\n");
  }
